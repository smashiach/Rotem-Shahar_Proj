...
import numpy as np
import os
from k_means import get_dominant_colors
...
def detect_faces(image_path, name):
    ...
    cv2.imwrite('extracted_' + name + '.jpg', sub_image_hsv)

    img_h, img_s, img_v = get_dominant_colors(sub_image_hsv, 'extracted_' + name + '.jpg')
    os.remove('extracted_' + name + '.jpg')
